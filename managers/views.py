...
def _build_commission_dataset(start_date, end_date, producer_filter="", status_filter=""):
    ...
    for order in qs:
        # Recompute the canonical 5% / 95% split from the recorded total.
        order_total = Decimal(order.total_amount or 0)
        # The stored total_amount already includes commission; the underlying
        # subtotal we split is therefore total / 1.05.
...
def _monthly_summary(months_back=6):
    ...
    for i in range(months_back):
        # Walk backwards month-by-month, anchoring on the 1st.
...
def financial_reports(request):
    ...
    # CSV export branch - identical filters, downloadable for accounting tools.
    if request.GET.get("export") == "csv":
        ...
        # Summary footer for accountants.
        writer.writerow([])
    ...
    # Status options pulled from the Order model when available.
    status_choices = []
    try:
        from basket.models import Order as OrderModel
        status_choices = OrderModel.STATUS_CHOICES
