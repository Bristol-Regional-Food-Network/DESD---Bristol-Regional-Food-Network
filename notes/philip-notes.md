...
The AI service is defined as a separate container in docker-compose.yml:
```yml
ai_service:
  build: ./ai_service
...
```
The Django service depends on it:
```yml
web:
  depends_on:
    - db
...
Django communicates with Flask using the internal Docker network:
AI_SERVICE_URL = `"http://ai_service:5001/predict"`

### Model Handling
The trained model (fruit_model.h5) is copied into the repository
No retraining is required at runtime
The model is loaded once when the Flask service starts

### Notes
- TensorFlow runs on CPU inside Docke
